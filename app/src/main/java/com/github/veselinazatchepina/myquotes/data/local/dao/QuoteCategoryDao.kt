...
interface QuoteCategoryDao {
    ...
    @Query("SELECT * FROM QuoteCategory INNER JOIN Quote ON QuoteCategory.categoryId = Quote.category_Id INNER JOIN (SELECT * FROM QuoteType WHERE QuoteType.type = :quoteType) c ON c.typeId = Quote.type_Id GROUP BY categoryId")
